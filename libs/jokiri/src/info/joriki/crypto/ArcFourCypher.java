/*
 * Copyright 2002 Felix Pahl. All rights reserved.
 * Use is subject to license terms.
 */

package info.joriki.crypto;

import info.joriki.util.Assertions;

public class ArcFourCypher extends SymmetricStreamCypher
{
  byte [] s = new byte [256];
  byte [] s0;
  int i,j;

  public ArcFourCypher (byte [] key)
  {
    this (key,0,key.length);
  }

  public ArcFourCypher (byte [] key,int off,int len)
  {
    for (int i = 0;i < 256;i++)
      s [i] = (byte) i;
    
    for (int i = 0,j = 0; i < 256;i++)
      {
        j += s [i] + key [off + (i % len)];
        j &= 0xff;
        swap (i,j);
      }
    s0 = s.clone ();
  }

  public void reset ()
  {
    i = j = 0;
    System.arraycopy (s0,0,s,0,256);
  }

  final private void swap (int i,int j)
  {
    byte swap = s [i];
    s [i] = s [j];
    s [j] = swap;
  }

  public byte nextByte ()
  {
    i++;
    i &= 0xff;
    j += s [i];
    j &= 0xff;
    swap (i,j);
    return s [(s [i] + s [j]) & 0xff];
  }

  protected byte crypt (int k)
  {
    return (byte) (k ^ nextByte ());
  }

  public void encrypt (byte [] buf)
  {
    for (int i = 0;i < buf.length;i++)
      buf [i] = encrypt (buf [i]);
  }

  public static int [] test1 = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  };
  public static int [] key1 = {
    0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF
  };
  public static int [] cypher1 = {
    0x74, 0x94, 0xC2, 0xE7, 0x10, 0x4B, 0x08, 0x79
  };

  public static int [] test2 = {
    0xdc, 0xee, 0x4c, 0xf9, 0x2c
  };
  public static int [] key2 = {
    0x61, 0x8a, 0x63, 0xd2, 0xfb
  };
  public static int [] cypher2 = {
    0xf1, 0x38, 0x29, 0xc9, 0xde
  };

  public static int [] test3 = {
    
  };
  public static int [] key3 = {
    0x29, 0x04, 0x19, 0x72, 0xfb, 0x42, 0xba, 0x5f,
    0xc7, 0x12, 0x77, 0x12, 0xf1, 0x38, 0x29, 0xc9
  };
  public static int [] cypher3 = {
    
  };
          
  public static int [] [] arrays = {
    test1,key1,cypher1,
    test2,key2,cypher2,
    test3,key3,cypher3
  };

  public static void main (String [] args)
  {
    byte [] [] barrays = new byte [arrays.length] [];
    for (int i = 0;i < arrays.length;i++)
      {
        barrays [i] = new byte [arrays [i].length];
        for (int j = 0;j < barrays [i].length;j++)
          barrays [i] [j] = (byte) arrays [i] [j];
      }

    for (int i = 0;i < barrays.length;i += 3)
      {
        System.out.println (i);
        ArcFourCypher cypher = new ArcFourCypher (barrays [i+1]);
        for (int j = 0;j < barrays [i].length;j++)
          Assertions.expect (cypher.encrypt (barrays [i] [j]),
                             barrays [i+2] [j]);
      }
    System.out.println (new String (barrays [6]));
  }
}
